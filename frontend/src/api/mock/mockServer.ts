...
const DISCLAIMER = "Bu tibbiy maslahat emas, almashtirishdan oldin shifokor yoki farmatsevt bilan maslahatlashing."
...
function buildPriceRows(drugId: number, lat: number, lng: number, radiusKm: number) {
  ...
  const rows = mockPrices
    ...
    .map((p) => {
      ...
      const deviationPercent = Math.round(((p.price - drug.reference_price) / drug.reference_price) * 100)
      return {
        ...
        is_overpriced: deviationPercent > OVERPRICE_THRESHOLD_PERCENT,
        updated_at: p.updated_at,
      }
    })
    .filter((row): row is NonNullable<typeof row> => row !== null && row.distance_km <= radiusKm)
...
export async function mockRequest<T>(path: string, options: MockRequestOptions = {}): Promise<T> {
  ...
  if (path === '/drugs/search' && method === 'GET') {
    ...
    const results: DrugSearchResult[] = drugs
      .filter((drug) => matchesQuery(drug, q))
      .map((drug) => ({
        id: drug.id,
        trade_name: drug.trade_name,
        substance_name: substanceName(drug.substance_id),
  ...
  if (pricesMatch && method === 'GET') {
    ...
    const response: DrugPricesResponse = {
      drug: { id: drug.id, trade_name: drug.trade_name, substance_name: substanceName(drug.substance_id) },
      prices: rows,
  ...
  if (altMatch && method === 'GET') {
    ...
    const drug = drugs.find((d) => d.id === drugId)
    if (!drug) throw new ApiError('Dori topilmadi.', 404)
    const alternatives: DrugAlternative[] = drugs
      .filter((d) => d.substance_id === drug.substance_id && d.id !== drug.id)
      .map((d) => {
        const drugPrices = mockPrices.filter((p) => p.drug_id === d.id && p.in_stock)
        const minPrice = drugPrices.length > 0 ? Math.min(...drugPrices.map((p) => p.price)) : null
        return {
          drug_id: d.id,
  ...
  if (path === '/prescriptions/scan' && method === 'POST') {
    ...
    const items: PrescriptionScanResponse['items'] = [
      { raw_text: first.trade_name, matched_drug_id: first.id, matched_trade_name: first.trade_name, confidence: 'high', confidence_score: 0.96 },
      { raw_text: second.aliases[0] ?? second.trade_name, matched_drug_id: second.id, matched_trade_name: second.trade_name, confidence: 'medium', confidence_score: 0.71 },
      { raw_text: "noaniq qo'lyozma...", matched_drug_id: null, matched_trade_name: null, confidence: 'low', confidence_score: 0.22 },
    ]
    return { items } as T
  }

  if (path === '/prescriptions/confirm' && method === 'POST') {
    const body = options.body as { items: { drug_id: number }[] }
    const results: PrescriptionConfirmResponse['results'] = body.items.map(({ drug_id }) => {
      const { drug, rows } = buildPriceRows(drug_id, TASHKENT_CENTER.lat, TASHKENT_CENTER.lng, 25)
      return { drug_id: drug.id, trade_name: drug.trade_name, prices: rows }
    })
    return { disclaimer: DISCLAIMER, results } as T
  }
  ...
  if (path === '/pharmacy/my/prices' && method === 'GET') {
    const user = requireRole('pharmacy_staff')
    const rows: PharmacyMyPriceRow[] = mockPrices
  ...
  if (path === '/pharmacy/my/prices' && method === 'POST') {
    ...
    const row: PharmacyMyPriceRow = { drug_id: drug.id, trade_name: drug.trade_name, price: body.price, in_stock: body.in_stock, updated_at: now }
    return row as T
  }
  ...
  if (path === '/pharmacy/my/prices/csv' && method === 'POST') {
    ...
    dataLines.forEach((line, index) => {
      ...
      if (columns.length < 3) {
        errors.push({ row: rowNumber, reason: 'Ustunlar yetarli emas (trade_name, price, in_stock kerak)' })
        return
      }
      ...
      if (!drug) {
        errors.push({ row: rowNumber, reason: `"${nameOrId}" nomli dori bazada topilmadi` })
        return
      }
      const price = Number(priceStr)
      if (!price || price <= 0 || Number.isNaN(price)) {
        errors.push({ row: rowNumber, reason: `Narx noto'g'ri: "${priceStr}"` })
        return
      }
    ...
    return { success_count: successCount, error_count: errors.length, errors } as T
  }

  if (path === '/admin/pharmacies' && method === 'GET') {
    requireRole('admin')
    const withStaffCount = mockPharmacies.map((p) => ({
      ...p,
      staff_count: mockUsers.filter((u) => u.pharmacy_id === p.id).length,
    }))
    return withStaffCount as T
  }
  ...
  if (path === '/admin/pharmacies' && method === 'POST') {
    ...
    const pharmacy: Pharmacy = { id: nextPharmacyId++, staff_count: 0, ...body }
  ...
  if (inviteMatch && method === 'POST') {
    ...
    if (!pharmacy) throw new ApiError('Dorixona topilmadi.', 404)
    const token = `invite-${pharmacyId}-${nextInviteToken++}`
    const response: PharmacyInviteResponse = { token, url: `https://t.me/apteka_bot?start=${token}` }
    return response as T
  }
